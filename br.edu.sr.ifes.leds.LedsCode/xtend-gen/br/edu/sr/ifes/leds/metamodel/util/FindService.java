...
public class FindService {
  public Method inMetaModel(final Set<Entity> entities, final RepositoryFields fields) {
    try {
  ...
  public Method inRepository(final Repository repository, final String name) {
    ...
    if (_notEquals) {
      boolean _and = false;
      LinkedHashSet<Method> _methods = repository.getMethods();
      boolean _notEquals_1 = (!Objects.equal(_methods, null));
      if (!_notEquals_1) {
        _and = false;
      } else {
        LinkedHashSet<Method> _methods_1 = repository.getMethods();
        int _size = _methods_1.size();
        boolean _greaterThan = (_size > 0);
        _and = _greaterThan;
      }
      if (_and) {
        LinkedHashSet<Method> _methods_2 = repository.getMethods();
